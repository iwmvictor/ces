  ...
  useEffect(() => {
    const fetchResolvedFeedbacks = async () => {
      try {
        ...
        // Filter only RESOLVED feedbacks
        const resolved = response.data.data.filter(
    ...
    <div className="agency-home">
      <div className="container">
        ...
        <div className="content">
          {loading ? (
            <p className="load">Loading resolved feedback...</p>
          ) : error ? (
            <p className="error">Error: {error}</p>
          ) : resolvedFeedbacks.length === 0 ? (
            <p className="empty">No resolved complaints found.</p>
          ) : (
            resolvedFeedbacks.map((item) => (
              <Link
